...
function StepRow({ order, description, duration }: ICookingStep) {
...
export const RecipeView = observer(({ recipe }: RecipeViewProps) => {
  const { t } = useTranslation();
  const {
    title,
    description,
  ...
  return (
    <ScrollShadow hideScrollBar className="flex flex-col gap-4 max-h-full">
